    ...
    public sealed class K3poControl
    {
        ...
        private PreparedEvent ReadPreparedEvent()
        {
            ...
            if (length > 0)
            {
                ...
                preparedEvent.Script = Encoding.UTF8.GetString(content, 0, length);
            }
        ...
        private ErrorEvent ReadErrorEvent()
        {
            ...
            {
                byte[] content = Read(length);
                errorEvent.Description = Encoding.UTF8.GetString(content, 0, length);
            }
        ...
        private FinishedEvent ReadFinishedEvent()
        {
            ...
            {
                byte[] content = Read(length);
                finishedEvent.Script = Encoding.UTF8.GetString(content, 0, length);
            }
        ...
        private string ReadLine()
        {
            ...
            while ((byteRead = _connection.ReadByte()) != 0x0A)
            {
        ...
        private byte[] Read(int length) {
            ...
            if (_connection.CanRead)
            {
                ...
                do
                {
                    bytesRead += _connection.Read(readBuffer, bytesRead, length - bytesRead);
